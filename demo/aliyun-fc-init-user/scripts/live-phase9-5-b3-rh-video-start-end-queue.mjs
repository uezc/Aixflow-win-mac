/**
 * Phase 9.5-B-3-B Live E2E — rh-video-start-end Unified Queue（真实 FC + OTS + RH.cn）
 * node scripts/live-phase9-5-b3-rh-video-start-end-queue.mjs
 */
...
const PREFIX = `__p95b3live_${Date.now().toString(36)}_`;
...
const SKU = 'ltx-2-3-start-end-720p-5s';
const OSS_IMAGE =
  process.env.P95B3_IMAGE_URL?.trim() ||
...
const OUT = path.join(__dirname, 'phase9-5-b3-rh-video-start-end-live-e2e-result.json');
...
function buildForward(prompt, startUrl, endUrl, duration = '5', resolution = '720p', aspect = '16:9') {
  const size =
    resolution === '720p'
      ? aspect === '9:16'
        ? { w: '720', h: '1280' }
  ...
  return {
    ...
    body: {
      nodeInfoList: [
        { nodeId: '2004', fieldName: 'image', fieldValue: startUrl, description: 'image1' },
        { nodeId: '5062', fieldName: 'image', fieldValue: endUrl, description: 'image2' },
        { nodeId: '5018', fieldName: 'value', fieldValue: size.w, description: '宽' },
        { nodeId: '5020', fieldName: 'value', fieldValue: size.h, description: '高' },
        { nodeId: '5022', fieldName: 'value', fieldValue: String(duration), description: '时长' },
        { nodeId: '5090', fieldName: 'text', fieldValue: prompt, description: 'text' },
      ],
      instanceType: 'plus',
      usePersonalQueue: 'false',
    },
    billingModelId: `ltx-2-3-start-end-${resolution}-${duration}s`,
    rhRegion: 'cn',
  };
}
...
async function main() {
  ...
  const otsPrice = getFinalPrice(SKU, { taskType: 'video', modelConfigMap });
  REPORT.ots_price = otsPrice;
  log(`OTS ${SKU}=${otsPrice}`);
  ...
  const forward = buildForward(
    'PHASE95B3 LTX start-end calm transition between two stills',
    OSS_IMAGE,
    `${OSS_IMAGE}?v=end`,
    '5',
    '720p',
  ...
  const cr = await fcFetch('/tasks/create', {
    ...
    body: {
      ...
      params: { model: 'rh-video-start-end', nxCloudQueueGoldenPath: true },
      nodeData: { model: 'rh-video-start-end', duration: '5', resolutionRhartV31: '720p' },
    },
  });
  ...
  const quoted = Number(cr.json.quoted_cost ?? cr.json.cost ?? 0) || Number(otsPrice);
  ...
  const liveOk =
    ...
    REPORT.live_ok.charge_count === 1 &&
    REPORT.live_ok.refund_count === 0;

  if (!liveOk) {
    REPORT.errors.push(`live fail status=${task?.status} err=${task?.error_code}`);
    if (task?.error_code === 'PROVIDER_NO_TASK_ID') {
  ...
  // Failure path: local mock submit fail after charge
  {
    ...
    const crFail = await handleTasksCreate(
      uFail,
      {
        ...
        type: 'video',
        execution_mode: 'queue',
        provider_forward_json: buildForward('fail', OSS_IMAGE, `${OSS_IMAGE}?e=2`),
        params: { model: 'rh-video-start-end', nxCloudQueueGoldenPath: true },
        nodeData: { model: 'rh-video-start-end' },
      },
      db,
      { getFinalPrice: () => otsPrice },
    );
  ...
  // local dups
  {
    ...
    const crDup = await handleTasksCreate(
      uDup,
      {
        ...
        type: 'video',
        execution_mode: 'queue',
        provider_forward_json: buildForward('dup', OSS_IMAGE, `${OSS_IMAGE}?e=3`),
        params: { model: 'rh-video-start-end', nxCloudQueueGoldenPath: true },
        nodeData: { model: 'rh-video-start-end' },
      },
      db,
    ...
    REPORT.regressions.duplicate_charge = Number(b0) - Number(b1) === Number(otsPrice) ? 'PASS' : 'FAIL';

    let rh = 0;
    await dispatchOneChargedTask(dTid, db, {
      submitRunningHub: async () => {
        rh += 1;
        return { ok: true, provider_task_id: `rh_${crypto.randomUUID()}`, data: {}, rhRegion: 'cn' };
      },
    });
    for (let i = 0; i < 10; i++) {
      await dispatchOneChargedTask(dTid, db, {
        submitRunningHub: async () => {
          rh += 1;
          return { ok: true, provider_task_id: 'x', data: {}, rhRegion: 'cn' };
        },
      });
    }
    REPORT.regressions.duplicate_dispatch = rh === 1 ? 'PASS' : `FAIL:${rh}`;
  ...
  const localOk =
    ...
    ['duplicate_charge', 'duplicate_refund', 'duplicate_dispatch', 'invalid_forward'].every(
      (k) => REPORT.regressions[k] === 'PASS',
    );

  REPORT.finished_at = new Date().toISOString();
  if (liveOk && localOk) REPORT.conclusion = 'PASS';
  else if (REPORT.external_blocker === 'BLOCKED_EXTERNAL' && localOk)
    REPORT.conclusion = 'NEEDS-REVIEW';
  else REPORT.conclusion = liveOk ? 'NEEDS-REVIEW' : 'FAIL';
