/**
 * Phase 9.5-B-7-B — light regression after seedance-2.0-mini
 * node scripts/live-phase9-5-b7-regression-only.mjs
 */
...
const PREFIX = `__p95b7reg_${Date.now().toString(36)}_`;
...
const OUT = path.join(__dirname, 'phase9-5-b7-regression-only-result.json');
const MAIN = path.join(__dirname, 'phase9-5-b7-seedance-mini-live-e2e-result.json');

function log(m) {
  console.log(`[p95b7-reg] ${m}`);
}
...
async function setBalance(userId, balance) {
  try {
    await db.createUserOtpOnly({ userId, email: `${userId}@p95b7reg.local` });
  } catch (_) {}
