/**
 * Phase 9.5-B-6-B — light regression after seedance-2.0-fast
 * node scripts/live-phase9-5-b6-regression-only.mjs
 */
...
const PREFIX = `__p95b6reg_${Date.now().toString(36)}_`;
...
const OUT = path.join(__dirname, 'phase9-5-b6-regression-only-result.json');
const MAIN = path.join(__dirname, 'phase9-5-b6-seedance-fast-live-e2e-result.json');

function log(m) {
  console.log(`[p95b6-reg] ${m}`);
}
...
async function setBalance(userId, balance) {
  try {
    await db.createUserOtpOnly({ userId, email: `${userId}@p95b6reg.local` });
  } catch (_) {}
...
async function main() {
  ...
  // duplicate charge / refund / dispatch (local mock, not RH)
  {
    ...
    const cr = await handleTasksCreate(
      u,
      {
        model_id: 'seedance-2-0-fast-720p-5s',
        type: 'video',
        execution_mode: 'queue',
        provider_forward_json: fwd,
        params: { model: 'seedance-2.0-fast', nxCloudQueueGoldenPath: true },
        nodeData: { model: 'seedance-2.0-fast', resolutionSeedance: '720p', durationSeedance: '5' },
      },
      db,
