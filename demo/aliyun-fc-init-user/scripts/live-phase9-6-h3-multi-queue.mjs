/**
 * Phase 9.6 Live E2E — minimax-h3-multi Unified Queue（真实 FC + OTS + RH.cn）
 * 仅烧 1 次 SKU：minimax-h3-multi-720p-6s
 *
 * node scripts/live-phase9-6-h3-multi-queue.mjs
 */
...
const PREFIX = `__p96live_${Date.now().toString(36)}_`;
...
const OSS_IMAGE =
  process.env.P96_MULTI_IMAGE_URL?.trim() ||
...
const OUT = path.join(__dirname, 'phase9-6-h3-multi-live-e2e-result.json');
  ...
  const res = await fcFetch('/run-task', {
    ...
    body: {
      ...
      forward: {
        ...
        uploadFromUrl: {
          url: imageUrl,
          filename: 'minimax-h3-multi-ref.jpg',
...
function buildH3MultiForward(prompt, rhImageField, durationSec = '6') {
  return {
    provider: 'runninghub',
    path: MULTI_PATH,
    method: 'POST',
    body: {
      nodeInfoList: [
        ...
        { nodeId: '28', fieldName: 'value', fieldValue: durationSec, description: '时长' },
        { nodeId: '18', fieldName: 'image', fieldValue: rhImageField, description: 'image1' },
      ],
      instanceType: 'plus',
      randomSeed: true,
      retainSeconds: 0,
    ...
    billingModelId: `minimax-h3-multi-720p-${durationSec}s`,
    rhRegion: 'cn',
  };
}
...
async function setBalance(userId, balance) {
  try {
    await db.createUserOtpOnly({ userId, email: `${userId}@p96live.local` });
  } catch (_) {}
...
async function runFcWorkersUntil(taskId, { maxRounds = 90, intervalMs = 8000 } = {}) {
  for (let i = 0; i < maxRounds; i++) {
    await fcInternal('run-queue-pipeline', { max_promote: 20, max_charge: 20, max_dispatch: 20, max_poll: 40 });
    const t = await db.getTaskById(taskId);
    log(
...
async function main() {
  ...
  // Queue-only source check
  {
    ...
    const ok =
      gp.includes("VIDEO_QUEUE_H3_MULTI_MODEL = 'minimax-h3-multi'") &&
  ...
  const prompt =
    'PHASE 9.6 MiniMax H3 Multi queue live, one reference image, gentle camera, soft light, no text overlay.';
  const forward = buildH3MultiForward(prompt, rhFile, '6');

  const createRes = await fcFetch('/tasks/create', {
    auth: token,
    body: {
      model_id: H3_MULTI_SKU,
      type: 'video',
      execution_mode: 'queue',
      provider_forward_json: forward,
      skip_queue_pipeline: true,
      params: { model: 'minimax-h3-multi', nxCloudQueueGoldenPath: true, prompt },
      nodeData: {
  ...
  if (fwd.rhRegion !== 'cn' || fwd.path !== MULTI_PATH || fwd.body?.instanceType !== 'plus') {
    REPORT.errors.push(`bad forward ${JSON.stringify(fwd).slice(0, 400)}`);
    fs.writeFileSync(OUT, JSON.stringify(REPORT, null, 2));
    process.exit(6);
  }

  log(`created task=${taskId} quoted=${quoted} qat=${t0?.queue_entered_at}`);
  ...
  const ok =
    ...
    REPORT.h3_multi_e2e.charge_count === 1 &&
    REPORT.h3_multi_e2e.refund_count === 0 &&
    REPORT.queue_only_check === 'PASS';

  REPORT.conclusion = ok ? 'PASS' : worker.timeout ? 'TIMEOUT' : 'FAIL';
  if (!ok && !REPORT.errors.length) {
    REPORT.errors.push(worker.timeout ? 'H3 Multi timeout' : 'H3 Multi e2e not success');
  }
  REPORT.finished_at = new Date().toISOString();
  fs.writeFileSync(OUT, JSON.stringify(REPORT, null, 2));
  log(`conclusion=${REPORT.conclusion} status=${final?.status} pid=${final?.provider_task_id} charge=${chargeAmt}`);
  console.log(JSON.stringify(REPORT, null, 2));
  process.exit(ok ? 0 : 1);
}
