/**
 * Phase 9.5-B-5-B — light regression after ltx-2.3-lipsync
 * node scripts/live-phase9-5-b5-regression-only.mjs
 */
...
const PREFIX = `__p95b5reg_${Date.now().toString(36)}_`;
...
const OUT = path.join(__dirname, 'phase9-5-b5-regression-only-result.json');
const MAIN = path.join(__dirname, 'phase9-5-b5-ltx-lipsync-live-e2e-result.json');

function log(m) {
  console.log(`[p95b5-reg] ${m}`);
}
...
async function setBalance(userId, balance) {
  try {
    await db.createUserOtpOnly({ userId, email: `${userId}@p95b5reg.local` });
  } catch (_) {}
  ...
  // duplicate charge / refund / dispatch (local mock, not RH)
  {
    ...
    const fwd = {
      ...
      body: {
        nodeInfoList: [
          { nodeId: '50', fieldName: 'image', fieldValue: OSS_IMAGE },
          { nodeId: '37', fieldName: 'audio', fieldValue: OSS_AUDIO.startsWith('https://') ? OSS_AUDIO : OSS_IMAGE },
          { nodeId: '54', fieldName: 'value', fieldValue: 'dup' },
          { nodeId: '187', fieldName: 'value', fieldValue: '720' },
        ],
        instanceType: 'plus',
        usePersonalQueue: 'false',
    ...
    const cr = await handleTasksCreate(
      u,
      {
        model_id: 'ltx-2-3-lipsync-720',
        type: 'video',
        execution_mode: 'queue',
        provider_forward_json: fwd,
        params: { model: 'ltx-2.3-lipsync', nxCloudQueueGoldenPath: true },
        nodeData: { model: 'ltx-2.3-lipsync', resolutionLtx23Lipsync: '720' },
      },
      db,
