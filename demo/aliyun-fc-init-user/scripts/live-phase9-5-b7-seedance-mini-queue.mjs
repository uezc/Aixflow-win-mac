/**
 * Phase 9.5-B-7-B Live E2E — seedance-2.0-mini Unified Queue
 * Live A: T2V 720p 5s；Live B: Image + Audio 720p 5s
 *
 * node scripts/live-phase9-5-b7-seedance-mini-queue.mjs
 */
...
const PREFIX = `__p95b7live_${Date.now().toString(36)}_`;
...
const RH_PATH = '/rhart-video/sparkvideo-2.0-mini/multimodal-video';
const MODEL = 'seedance-2.0-mini';
const SKU_720_5 = 'seedance-2-0-mini-720p-5s';
const OSS_IMAGE =
  process.env.P95B7_IMAGE_URL?.trim() ||
...
const OSS_AUDIO =
  process.env.P95B7_AUDIO_URL?.trim() ||
  (fs.existsSync(path.join(__dirname, 'p95b5-lipsync-audio-url.txt'))
    ...
    : '') ||
  '';
const OUT = path.join(__dirname, 'phase9-5-b7-seedance-mini-live-e2e-result.json');
...
const REPORT = {
  ...
  errors: [],
  live_a_t2v: null,
  live_b_image_audio: null,
  live_fail_refund: null,
  external_blocker: null,
};

function log(msg) {
  console.log(`[p95b7-live] ${msg}`);
}
...
function buildForward(prompt, { imageUrls = [], videoUrls = [], audioUrls = [], resolution = '720p', duration = '5', ratio = 'adaptive' } = {}) {
  const res = ['480p', '720p', '1080p', '2k', '4k'].includes(resolution) ? resolution : '720p';
...
function summarizeLive(task, fwd, charge, bal0, bal1, quoted, expectSku, expectCharge) {
  return {
    ...
    audio_count: Array.isArray(fwd.body?.audioUrls) ? fwd.body.audioUrls.length : null,
    conversion_slots: fwd.body?.conversionSlots || null,
    quoted_cost: quoted,
...
async function main() {
  ...
  {
    const fwd = buildForward('PHASE95B7-A Seedance Mini T2V calm cinematic shot', {
      imageUrls: [],
      resolution: '720p',
      duration: '5',
    });
  ...
  {
    if (!OSS_AUDIO || !OSS_AUDIO.startsWith('https://') || OSS_AUDIO.includes('sample-audio.mp3')) {
      REPORT.live_b_image_audio = {
        ok: false,
        skipped: true,
        note: 'NO_REAL_AUDIO_URL (set P95B7_AUDIO_URL or p95b5-lipsync-audio-url.txt)',
      };
      REPORT.errors.push('liveB missing real audio URL');
      log('liveB skipped: missing real audio URL');
    } else {
      const fwd = buildForward('PHASE95B7-B Seedance Mini image+audio', {
        imageUrls: [OSS_IMAGE],
        audioUrls: [OSS_AUDIO],
        resolution: '720p',
        duration: '5',
      ...
      REPORT.live_b_image_audio = {
        ...
        ...r.summary,
        error: r.error,
        conversion_slots_ok: Array.isArray(r.summary?.conversion_slots) && r.summary.conversion_slots[0] === 'all',
      };
  ...
  {
    const u = `${PREFIX}fail`;
    const tok = await setupUser(u, 500);
    const bal0 = (await db.getUserById(u)).balance;
    const fwd = buildForward('PHASE95B7 fail refund', { resolution: '720p', duration: '5' });
    // force bad path to trigger RH create fail after charge: use invalid body image that RH rejects
    fwd.body.imageUrls = ['https://example.invalid/nexflow-force-fail.jpg'];
    const cr = await fcFetch('/tasks/create', {
      auth: tok,
      body: {
        model_id: SKU_720_5,
    ...
    if (cr.status === 200 && cr.json?.task_id) {
      ...
      await runFcWorkersUntil(tid, { maxRounds: 20, intervalMs: 5000 });
      const final = await db.getTaskById(tid);
      const bal1 = (await db.getUserById(u)).balance;
      const ok =
        ['failed', 'cancelled', 'timeout'].includes(String(final?.status || '').toLowerCase()) &&
        Number(bal1) === Number(bal0);
      REPORT.live_fail_refund = { ok, status: final?.status, bal0, bal1 };
      log(`fail_refund ok=${ok}`);
    } else {
      REPORT.live_fail_refund = { ok: false, note: 'create failed', create: cr.json };
    }
  }

  const aOk = REPORT.live_a_t2v?.ok === true || REPORT.live_a_t2v?.external === true;
  const bOk =
    REPORT.live_b_image_audio?.ok === true ||
    REPORT.live_b_image_audio?.external === true ||
    REPORT.live_b_image_audio?.skipped === true;
  const failOk = REPORT.live_fail_refund?.ok !== false;
  REPORT.conclusion =
    aOk && REPORT.live_b_image_audio?.ok === true && REPORT.live_fail_refund?.ok === true
      ? 'PASS'
      : aOk && REPORT.live_b_image_audio?.ok === true
        ? 'PASS'
        : REPORT.errors.length
          ? 'FAIL'
          : 'NEEDS-REVIEW';
  if (REPORT.live_b_image_audio?.skipped) {
    REPORT.conclusion = aOk && REPORT.live_fail_refund?.ok ? 'NEEDS-REVIEW' : REPORT.conclusion;
  }
