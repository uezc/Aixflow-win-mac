/**
 * Phase 9.9 Live E2E — rhart-video-upscaler Unified Queue
 * 仅烧 1 次真实 RH（最低成本：720p × 最短计费 5s）
 * 另做本地可控 failure/refund（不二次烧 RH）
 *
 * node scripts/live-phase9-9-rhart-video-upscaler-queue.mjs
 */
...
const PREFIX = `__p99live_${Date.now().toString(36)}_`;
...
const OSS_VIDEO =
  process.env.P99_VIDEO_URL?.trim() ||
  (fs.existsSync(path.join(__dirname, 'p95b8-wan-animate-video-url.txt'))
    ...
    : '');
/** 最短计费秒数（与 normalizeRhartVideoUpscalerBillingSec 一致） */
const MEDIA_SEC = Math.max(5, Math.floor(Number(process.env.P99_MEDIA_DURATION_SEC || 5) || 5));
const OUT = path.join(__dirname, 'phase9-9-rhart-video-upscaler-live-e2e-result.json');
...
function buildUpsForward(videoUrl, targetResolution = '720p') {
  return {
    provider: 'runninghub',
    path: UPS_PATH,
    method: 'POST',
...
async function setBalance(userId, balance) {
  try {
    await db.createUserOtpOnly({ userId, email: `${userId}@p99live.local` });
  } catch (_) {}
...
async function main() {
  ...
  if (!OSS_VIDEO.startsWith('https://')) {
    REPORT.errors.push(`bad OSS video=${OSS_VIDEO.slice(0, 40)}`);
    fs.writeFileSync(OUT, JSON.stringify(REPORT, null, 2));
    process.exit(4);
  }

  {
    ...
    const ok =
      gp.includes("VIDEO_QUEUE_RHART_VIDEO_UPSCALER_MODEL = 'rhart-video-upscaler'") &&
      !!onlyBlock &&
      ...
      !/hey-gem/.test(onlyBlock[1] || '') &&
      vp.includes('executeRhartVideoUpscalerCloudQueueGoldenPath') &&
      vp.includes('rhart-video-upscaler 已强制云端排队，禁止 Direct');
  ...
  const userId = `${PREFIX}ups`;
  const initial = 2000;
  const token = await setupUser(userId, initial);
  const forward = buildUpsForward(OSS_VIDEO, '720p');
  ...
  REPORT.upscaler_e2e = {
    ...
    path: UPS_PATH,
    category: 'video_processing_upscale',
...

  {
    ...
    const cr = await handleTasksCreate(
      uFail,
      {
        ...
        type: 'video',
        execution_mode: 'queue',
        provider_forward_json: buildUpsForward(OSS_VIDEO, '720p'),
      ...
      db,
      { getFinalPrice: () => Number(quoted) || 14 },
    );
    ...
    await dispatchOneChargedTask(failId, db, {
      submitRunningHub: async () => ({ ok: false, reason: 'P99_LIVE_CTRL_FAIL' }),
    });
  ...
  if (!liveOk && !REPORT.errors.length) {
    REPORT.errors.push(worker.timeout ? 'upscaler timeout' : 'upscaler e2e not success');
  }
