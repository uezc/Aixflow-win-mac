/**
 * Phase 9.5-B-3-B — light regression after rh-video-start-end PASS
 * node scripts/live-phase9-5-b3-regression-only.mjs
 */
...
const PREFIX = `__p95b3reg_${Date.now().toString(36)}_`;
...
const OUT = path.join(__dirname, 'phase9-5-b3-regression-only-result.json');
const MAIN = path.join(__dirname, 'phase9-5-b3-rh-video-start-end-live-e2e-result.json');

function log(m) {
  console.log(`[p95b3-reg] ${m}`);
}
...
async function setBalance(userId, balance) {
  try {
    await db.createUserOtpOnly({ userId, email: `${userId}@p95b3reg.local` });
  } catch (_) {}
...
async function main() {
    ...
    'ltx-2-3-720-10s',
    {
      ...
      body: {
        nodeInfoList: [
          ...
          { nodeId: '73', fieldName: 'text', fieldValue: 'P95B3reg LTX T2V' },
        ],
        instanceType: 'plus',
    ...
    'ltx-2-3-i2v-720-10s',
    {
      ...
      body: {
        nodeInfoList: [
          ...
          { nodeId: '602', fieldName: 'positive', fieldValue: 'P95B3reg LTX I2V' },
        ],
        instanceType: 'plus',
        usePersonalQueue: 'false',
      },
  ...
  REPORT.results.h3_t2v = await quick(
    ...
    'minimax-h3-t2v-720p-10s',
    {
      ...
      body: {
        nodeInfoList: [
          { nodeId: '149', fieldName: 'text', fieldValue: 'P95B3reg H3 T2V', description: '提示词' },
    ...
    'minimax-h3-i2v-720p-10s',
    {
      ...
      body: {
        nodeInfoList: [
          ...
          { nodeId: '149', fieldName: 'text', fieldValue: 'P95B3reg H3 I2V', description: '提示词' },
        ],
        instanceType: 'plus',
        usePersonalQueue: 'false',
      },
  ...
  REPORT.results.rhart_t2v = await quick(
    ...
    'rhart-video-x-720p-10s',
    {
      ...
      method: 'POST',
      body: { prompt: 'P95B3reg rhart t2v', aspectRatio: '16:9', resolution: '720p', duration: 10 },
